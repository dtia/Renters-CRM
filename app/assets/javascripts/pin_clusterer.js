(function () {
	...
	PinClusterer.prototype = {
		...
		_addToClosestCluster: function (latlong) {
			...
			if (this._zoom >= _defaults.maxZoom) {
				...
				this._createCluster(location, title, description);
				return;
			}
			...
			if (clusterToAddTo && clusterToAddTo.containsWithinBorders(location)) {
				clusterToAddTo.add(location, title, description);
			} else {
				this._createCluster(location, title, description);
			}
		},
		
		_createCluster: function (location, title, description) {
			var cluster = new Cluster(this);
			cluster.add(location, title, description);
	...
	Cluster.prototype = {
		add: function (location, title, description) {
		  if (this._alreadyAdded(location)) return;
			this.locations.push(location);
			this.length += 1;
			if (!this.center) {
				this.center = new Pin(location, this, { "title": title, "description": description });
	...
	Pin.prototype = {
		...
		_create: function () {
			...
			// infobox content
			this.pushpin.title = this._options.title;
			this.pushpin.description = this._options.description;
			
			// Create the infobox for the pushpin
		    var pinInfobox = new Microsoft.Maps.Infobox(this.pushpin.getLocation(), {visible:false});
			infoboxLayer.push(pinInfobox);
			currentPinInfobox = pinInfobox;

			 // Add handler for the pushpin click event.
		    Microsoft.Maps.Events.addHandler(this.pushpin, 'click', displayInfobox.bind(pinInfobox));
